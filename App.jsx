...
//                         testKey() updated to test Anthropic endpoint for Key B slot
...
// AI ENGINE       : Primary: gemini-2.5-flash-lite (Gemini, user key optional) | Secondary: claude-sonnet-4-20250514 (Anthropic, user key required)
...
// ─── API KEY CONFIG ───────────────────────────────────────────────────────────
const KEY_A_DEFAULT = "";   // Gemini key — optional; leave blank if not using Gemini
const KEY_B_DEFAULT = "";   // Anthropic key — user must provide via Admin → API Keys
const KEY_A_MODEL   = "gemini-2.5-flash-lite";    // Gemini 2.5 Flash Lite (stable GA — July 2025)
const KEY_B_MODEL   = "claude-sonnet-4-20250514"; // Claude Sonnet 4 (user-provided Anthropic key)
...
// ─── CLAUDE API (User-provided Anthropic API key — no built-in proxy) ────────
// Calls the Anthropic Messages API directly using the user's key from API Keys page.
...
async function claudeGenerateRaw(prompt, apiKey) {
  if (!apiKey || apiKey.trim().length < 10) {
    throw Object.assign(
      new Error("Anthropic API key not configured — enter your key in Admin → API Keys → Key B"),
      { phase: "no_key" }
    );
  }
  let lastError;
  for (const model of CLAUDE_MODEL_FALLBACKS) {
    try {
      ...
      if (e.status === 404) { lastError = e; continue; }
      throw e;
    }
  }
...
async function aiGenerate(prompt) {
  ...
  if (!isPrimA) {
    // Key B selected → Claude Sonnet with user-provided Anthropic key
    return claudeGenerateRaw(prompt, keys.keyB);
  }
  const primaryKey = keys.keyA;
  if (!primaryKey) {
    // No Gemini key configured → fall back to Claude with user key
    return claudeGenerateRaw(prompt, keys.keyB);
  }
  // ── SECONDARY PROMOTE ──────────────────────────────────────────────────────
  // If Gemini key is throttled (429 / rate-limited), silently promote to
  // Claude (user key) instead of throwing — zero interruption to the user.
  if (isKeyThrottled(primaryKey)) {
    console.warn("[ThreatPrep] Gemini key throttled — promoting to Claude (secondary).");
    return claudeGenerateRaw(prompt, keys.keyB);
  }
  // Primary path: Gemini
  try {
    ...
    if (e.status === 429) {
      ...
      console.warn("[ThreatPrep] Gemini 429 — promoting to Claude (secondary).");
      return claudeGenerateRaw(prompt, keys.keyB);
    }
...
function AdminPanel({ onClose, onQbaseChange, fetchContext }) {
  ...
  const tiles = [
    ...
    {id:"apikeys",  icon:"🔑",  label:"API Keys",           desc:"Key A: Gemini (optional) · Key B: Claude Sonnet (user Anthropic key)"},
...
function APIKeysPage({ onBack }) {
  ...
  const testKey = async (slot) => {
    ...
    if (!key || key.trim().length < 10) { setTestResult(r=>({...r,[slot]:{status:"error",msg:"No key entered"}})); return; }
    setTesting(t=>({...t,[slot]:true}));
    setTestResult(r=>({...r,[slot]:{status:"testing",msg:"Testing…"}}));
    try {
      if (slot === "b") {
        // Test Anthropic API with user-provided key
        const resp = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({ model:KEY_B_MODEL, max_tokens:10, messages:[{role:"user",content:"Reply with exactly: OK"}] }),
        });
        if (resp.ok) {
          const data = await resp.json();
          const text = ((data.content||[]).filter(b=>b.type==="text").map(b=>b.text)).join("").trim();
          setTestResult(r=>({...r,[slot]:{status:"ok",msg:`✓ Connected · Model: ${KEY_B_MODEL} · Response: "${text.slice(0,20)}"`}}));
        } else {
        ...
        // Test Gemini API
        const url = GEMINI_ENDPOINT(KEY_A_MODEL, key);
        ...
        if (resp.ok) {
          ...
          setTestResult(r=>({...r,[slot]:{status:"ok",msg:`✓ Connected · Model: ${KEY_A_MODEL} · Response: "${text.slice(0,20)}"`}}));
        } else {
          const err = await resp.json().catch(()=>({}));
          setTestResult(r=>({...r,[slot]:{status:"error",msg:`✗ HTTP ${resp.status}: ${err?.error?.message||"Unknown error"}`}}));
        }
  ...
  return (
    <div className="admin-overlay">
      <div className="admin-header">
        <button onClick={onBack} style={{ width:28,height:28,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",background:"transparent",border:"1px solid var(--border)",color:"var(--text-sec)",cursor:"pointer",fontSize:13,fontWeight:700 }}>←</button>
        <span style={{ fontSize:16 }}>🔑</span>
        <div style={{ flex:1,minWidth:0 }}><div className="admin-title">API Keys</div><div className="admin-sub">Gemini (Key A, optional) · Claude Sonnet (Key B, required)</div></div>
      </div>
      <div className="admin-body">
        <div style={{ padding:"10px 12px",background:"var(--accent-bg)",border:"1px solid var(--accent-bd)",borderRadius:"var(--r-sm)",marginBottom:14,fontSize:11,color:"var(--text-sec)",lineHeight:1.6 }}>
          <strong style={{ color:"var(--text-pri)" }}>Key A (Gemini):</strong> Optional. Leave blank to skip Gemini and use Claude exclusively.<br/>
          <strong style={{ color:"var(--text-pri)" }}>Key B (Claude):</strong> Required for AI features. Enter your Anthropic API key from <span style={{ color:"var(--accent)" }}>console.anthropic.com</span>.
        </div>
        ...
        {/* Key B — Claude Sonnet (User-provided Anthropic API key) */}
        {(()=>{
          const slot = "b";
          const isPrimary = keys.primaryKey === slot;
          const tr = testResult[slot];
          return (
            <div style={{ marginBottom:12,padding:"12px 14px",background:"var(--surface)",border:`1px solid ${isPrimary?"var(--accent-bd)":"var(--border)"}`,borderRadius:"var(--r-sm)" }}>
              ...
              <div style={{ display:"flex",gap:6,marginBottom:8 }}>
                <input type={showB?"text":"password"} value={keys.keyB} onChange={e=>setKeys(k=>({...k,keyB:e.target.value}))} placeholder="Anthropic API key (console.anthropic.com)" style={{ flex:1 }} />
                <button onClick={()=>setShowB(v=>!v)} style={{ width:36,height:34,borderRadius:"var(--r-sm)",border:"1px solid var(--border)",background:"var(--elevated)",color:"var(--text-sec)",cursor:"pointer",fontSize:14,flexShrink:0 }}>{showB?"🙈":"👁"}</button>
              </div>
              <button onClick={()=>testKey(slot)} disabled={testing[slot]} style={{ width:"100%",padding:"6px",borderRadius:"var(--r-sm)",border:"1px solid var(--border)",background:"var(--elevated)",color:"var(--text-sec)",fontSize:10,fontFamily:"var(--font-mono)",cursor:"pointer",fontWeight:600 }}>
                {testing[slot]?"⟳ Testing…":"⚡ Test Key"}
              </button>
              ...
              <div style={{ marginTop:8,fontSize:9,color:"var(--text-sec)",fontFamily:"var(--font-mono)",lineHeight:1.6 }}>
                Get an API key at <span style={{ color:"var(--accent)" }}>console.anthropic.com</span> — requires Anthropic account.
              </div>
            </div>
          );
        })()}
